...
export default function PDFPreview() {
  ...
  const canCustomizePdf = hasFeature('pdf_customization');
  ...
  useEffect(() => {
    if (!canCustomizePdf && template !== 'railway') {
      setTemplate('railway');
    }
  }, [canCustomizePdf, template]);
        ...
        >
          ...
          {!isMobile && (
            <select
              ...
              disabled={!canCustomizePdf}
              title={!canCustomizePdf ? 'Personalização de PDF disponível no Studio' : undefined}
              style={{
                padding: '6px 12px',
                borderRadius: '6px',
                border: '1px solid #2a2a2a',
